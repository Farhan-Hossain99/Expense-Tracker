...
function initDoughnutChart() {
    ...
    spendingChart = new Chart(ctx, {
        ...
        options: { responsive: true, maintainAspectRatio: true, cutout: '65%', plugins: { legend: { display: false }, tooltip: { callbacks: { label: function(context) { const value = context.raw; const total = context.dataset.data.reduce((a, b) => a + b, 0); const pct = total > 0 ? ((value / total) * 100).toFixed(0) : 0; return `$${value.toFixed(2)} (${pct}%)`; } } } } }
    });
}
...
function updateDoughnutChart(data) {
    ...
    if (!data || !data.by_category || data.by_category.length === 0) {
        if (spendingChart) { spendingChart.data.labels = []; spendingChart.data.datasets[0].data = []; spendingChart.data.datasets[0].backgroundColor = []; spendingChart.update(); }
...
function updateSummaryCards(data, income) {
    ...
    if (data.by_category && data.by_category.length > 0) {
        ...
        Object.keys(aggregatedCategories).forEach(cat => { const pct = total > 0 ? ((aggregatedCategories[cat] / total) * 100).toFixed(0) : 0; const item = document.querySelector(`[data-category-legend="${cat}"] .category-pct`); if (item) item.textContent = `${pct}%`; });
...
function updateTransactionsList(expenses) {
    ...
    const categoryIcons = { 'Food': 'restaurant', 'Transport': 'directions_car', 'Shopping': 'shopping_bag', 'Bills': 'receipt_long', 'Entertainment': 'movie', 'Health': 'health_and_safety', 'Other': 'more_horiz' };
    const categoryColors = { 'Food': 'orange-500', 'Transport': 'blue-500', 'Shopping': 'green-500', 'Bills': 'pink-500', 'Entertainment': 'pink-500', 'Health': 'green-500', 'Other': 'gray-500' };
...
function filterTransactions(searchTerm) {
    ...
    if (!search) { searchResultsSection.classList.add('hidden'); items.forEach(item => item.style.display = ''); return; }
    const matchingExpenses = [];
    items.forEach(item => {
        const searchText = item.getAttribute('data-search-text') || '';
        const matches = searchText.includes(search);
        item.style.display = matches ? '' : 'none';
        if (matches) { const exp = { description: item.querySelector('p.font-bold')?.textContent || '', category: item.getAttribute('data-category') || '', amount: parseFloat(item.querySelector('span.font-bold')?.textContent.replace('$','').replace('-','') || 0), date: item.getAttribute('data-date') || new Date().toISOString().split('T')[0] }; matchingExpenses.push(exp); }
    });
...
async function handleAddExpense(event) {
    ...
    if (!CORE_CATEGORIES.includes(category)) category = 'Other';
    if (!amount || !category || !date) { alert('Please fill in all required fields'); return; }
    submitBtn.disabled = true;
    submitBtn.textContent = 'Adding...';
    try { const response = await fetch(`${API_BASE}/expenses`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ amount, category, description, date }) });
        if (response.ok) { closeAddExpenseModal(); loadDashboardData(); } else { const error = await response.json(); alert(`Error: ${error.error}`); }
    } catch (error) { alert('Failed to add expense. Is the server running?'); }
...
function updateSavingsGoals(goals) {
    ...
    if (!container) return;
    if (!goals || goals.length === 0) { container.innerHTML = '<p class="text-sm text-gray-500 text-center py-4">No savings goals yet</p>'; return; }
    container.innerHTML = goals.map(goal => { const progress = ((goal.current_amount / goal.target_amount) * 100).toFixed(1); return `<div class="flex flex-col gap-2 p-4 bg-gray-50 rounded-xl"><div class="flex justify-between items-center"><span class="font-bold text-lg text-gray-800">${goal.name}</span><span class="text-sm text-gray-500">${goal.deadline || 'No deadline'}</span></div><div class="h-2 bg-gray-200 rounded-full overflow-hidden"><div class="h-full bg-green-500 rounded-full transition-all" style="width: ${Math.min(progress, 100)}%"></div></div><div class="flex justify-between"><span class="text-sm text-green-600">$${goal.current_amount.toFixed(2)}</span><span class="text-sm text-gray-500">of $${goal.target_amount.toFixed(2)}</span></div></div>`; }).join('');
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (modal) { modal.addEventListener('click', (e) => { if (e.target === modal) closeAddExpenseModal(); }); }
    const searchInput = document.getElementById('search-input');
    if (searchInput) { searchInput.addEventListener('input', (e) => { filterTransactions(e.target.value); }); }
    const incomeModal = document.querySelector('[data-income-modal]');
